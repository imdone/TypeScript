...
module TypeScript {
    ...
    export function preCollectImportTypes(ast: AST, parent: AST, context: TypeCollectionContext) {
        ...
        // REVIEW: technically, this call isn't strictly necessary, since we'll find the type during the call to resolveTypeMembers id:752 gh:753
    ...
    export function preCollectModuleTypes(ast: AST, parent: AST, context: TypeCollectionContext) {
        ...
        // REVIEW: If multiple disparate module decls for the same module don't agree id:429 gh:430
    ...
    export function preCollectInterfaceTypes(ast: AST, parent: AST, context: TypeCollectionContext) {
        ...
        if (interfaceSymbol == null) {
            ...
            // REVIEW: Shouldn't allocate another table for interface privates id:573 gh:574
        ...
        if (!alreadyInScope) {
            context.scopeChain.scope.enter(context.scopeChain.container, ast,
                                            interfaceSymbol, context.checker.errorReporter, isGlobal || isExported, true, false); // REVIEW: Technically, interfaces should be ambient id:426 gh:427
        }
    ...
    export function preCollectFuncDeclTypes(ast: AST, parent: AST, context: TypeCollectionContext) {
        ...
        // REVIEW: This will have to change when we move to "export" id:428 gh:429
        if (context.scopeChain.moduleDecl) {
            context.scopeChain.moduleDecl.recordNonInterface();
        }
        ...
        if (hasFlag(funcDecl.fncFlags, FncFlags.Signature)) {
            ...
            if (nameText && nameText != "__missing") {
                if (isStatic) {
                    ...
                    // REVIEW: This logic should be symmetric with preCollectClassTypes id:753 gh:754
            ...
            // REVIEW: In the code below, we need to ensure that only function overloads are considered id:432 gh:433
            ...
            // REVIEW: Move this check into the typecheck phase?  It's only being run over properties... id:574 gh:575
            if (fgSym &&
                !fgSym.isAccessor() &&
                fgSym.type &&
            ...
            if (funcDecl.isConstructor) { // REVIEW: Remove when classes completely replace oldclass id:430 gh:431
                go = true;
            };
        }
        if (isExported) {
...
(function (TypeScript) {
    ...
    function preCollectImportTypes(ast, parent, context) {
        ...
        var importDecl = ast;
        var isExported = hasFlag(importDecl.varFlags, VarFlags.Exported);
        // REVIEW: technically, this call isn't strictly necessary, since we'll find the type during the call to resolveTypeMembers id:431 gh:432
        var aliasedModSymbol = findSymbolFromAlias(importDecl.alias, { topLevelScope: scopeChain, members: null, tcContext: context });
        var isGlobal = context.scopeChain.container == context.checker.gloMod;
        if (aliasedModSymbol) {
            var aliasedModType = aliasedModSymbol.getType();
    ...
    function preCollectModuleTypes(ast, parent, context) {
        ...
        // REVIEW: If multiple disparate module decls for the same module don't agree id:754 gh:755
        // in export privileges, how should we handle it?
        if (isExported) {
            typeSymbol.flags |= SymbolFlags.Exported;
        }
    ...
    function preCollectInterfaceTypes(ast, parent, context) {
        ...
        if (interfaceSymbol == null) {
            ...
            interfaceType.symbol = interfaceSymbol;
            // REVIEW: Shouldn't allocate another table for interface privates id:435 gh:436
            interfaceType.members = new ScopedMembers(new DualStringHashTable(new StringHashTable(), new StringHashTable()));
            interfaceType.ambientMembers = new ScopedMembers(new DualStringHashTable(new StringHashTable(), new StringHashTable()));
            interfaceSymbol.declAST = interfaceDecl;
            interfaceSymbol.declModule = context.scopeChain.moduleDecl;
        ...
        if (!alreadyInScope) {
            context.scopeChain.scope.enter(context.scopeChain.container, ast, interfaceSymbol, context.checker.errorReporter, isGlobal || isExported, true, false); // REVIEW: Technically, interfaces should be ambient id:575 gh:576
        }
    ...
    function preCollectFuncDeclTypes(ast, parent, context) {
        var scopeChain = context.scopeChain;
        // REVIEW: This will have to change when we move to "export" id:434 gh:435
        if (context.scopeChain.moduleDecl) {
            context.scopeChain.moduleDecl.recordNonInterface();
        }
        var funcDecl = ast;
        ...
        if (hasFlag(funcDecl.fncFlags, FncFlags.Signature)) {
            ...
            if (nameText && nameText != "__missing") {
                if (isStatic) {
                    ...
                    // REVIEW: This logic should be symmetric with preCollectClassTypes id:433 gh:434
            ...
            // a function with this symbol has not yet been declared in this scope
            // REVIEW: In the code below, we need to ensure that only function overloads are considered id:755 gh:756
            //  (E.g., if a vardecl has the same id as a function or class, we may use the vardecl symbol
            //  as the overload.)  Defensively, however, the vardecl won't have a type yet, so it should
            //  suffice to just check for a null type when considering the overload symbol in
            //  createFunctionSignature
            ...
            // REVIEW: Move this check into the typecheck phase?  It's only being run over properties... id:438 gh:439
            if (fgSym &&
                !fgSym.isAccessor() &&
                fgSym.type &&
                fgSym.type.construct &&
